...
const Bills = () => {
  ...
  useEffect(() => {
    if (urlOrderId && bills.length > 0 && !hasAutoSelected.current) {
      const bill = bills.find(b => b.orderId?._id === urlOrderId || b.orderId === urlOrderId);
      if (bill) {
        handleViewBill(bill);
  ...
  return (
    <>
      <div className="print:hidden">
        ...
        {/* Header */}
        <div>
          {user?.role === 'superadmin' && !urlRestaurantId && !urlOrderId && (
            <div className="bg-orange-100 border-l-4 border-orange-500 p-4 mb-6 rounded-r-xl">
              <p className="text-orange-700 font-medium">
                You are viewing bills as a Superadmin. To manage a specific restaurant, 
                please go to the <Link to="/superadmin/restaurants" className="underline font-bold hover:text-orange-800">Restaurants</Link> page 
                and click the <Receipt size={16} className="inline mx-1" /> icon.
              </p>
            </div>
          )}
                ...
                bills
                  .filter((bill) => {
                    ...
                    return (
                      ...
                      bill.orderId?.customerName?.toLowerCase().includes(search) ||
                      bill.orderId?.tableNumber?.toString().includes(search)
                    );
                  })
                  .map((bill) => (
                  <tr key={bill._id}>
                    ...
                    <td>
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg font-medium transition-all duration-200"
                          title="View Bill"
                        >
                          <Eye size={16} />
                          <span>View</span>
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 text-orange-600 hover:bg-orange-600 hover:text-white rounded-lg font-medium transition-all duration-200"
                          title="Edit Bill"
                        >
                          <Edit size={16} />
                          <span>Edit</span>
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 text-green-600 hover:bg-green-600 hover:text-white rounded-lg font-medium transition-all duration-200"
      ...
      {selectedBill && !showEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 border-t flex flex-row-reverse gap-3 sticky bottom-0 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
              <button
                onClick={() => window.print()}
                className="flex-1 py-3 bg-primary text-white font-bold rounded-xl shadow-lg shadow-primary/20 hover:bg-primary-dark transition-all flex items-center justify-center gap-2"
              >
                <Printer size={20} />
                Print Bill
              </button>
      ...
      {selectedBill && (
        <div className="hidden print:block w-[80mm] mx-auto p-4 bg-white text-black font-mono text-sm leading-tight">
          <div className="text-center mb-4">
            <h2 className="font-bold text-xl mb-1">{selectedBill.restaurantId?.name || 'Restaurant'}</h2>
            <p className="text-xs">{selectedBill.restaurantId?.address || ''}</p>
            {selectedBill.restaurantId?.phone && <p className="text-xs">Tel: {selectedBill.restaurantId.phone}</p>}
            {selectedBill.gstin && <p className="text-xs font-bold mt-1">GSTIN: {selectedBill.gstin}</p>}
          </div>

          <div className="border-b border-dashed border-gray-400 mb-2 pb-2 text-xs">
            <p>Bill No: {selectedBill.billNumber}</p>
            <p>Date: {new Date(selectedBill.createdAt).toLocaleString('en-IN')}</p>
            <p>Type: {selectedBill.orderId?.orderType?.replace('_', ' ').toUpperCase() || 'DINE IN'}</p>
            {selectedBill.orderId?.tableNumber && <p>Table: {selectedBill.orderId.tableNumber}</p>}
          </div>

          <table className="w-full text-xs box-border mb-2">
            <thead>
              <tr className="border-b border-dashed border-gray-400">
                <th className="text-left py-1 font-bold">Qty Item</th>
          ...
          <div className="border-t border-dashed border-gray-400 pt-2 text-xs space-y-1">
            ...
            {selectedBill.tax > 0 && (
              <>
                <div className="flex justify-between">
                  <span>{selectedBill.taxLabel === 'GST' ? 'CGST' : `C-${selectedBill.taxLabel}`} ({(selectedBill.taxRate / 2).toFixed(1)}%):</span>
                  <span>{formatCurrency(selectedBill.tax / 2)}</span>
                </div>
                <div className="flex justify-between">
                  <span>{selectedBill.taxLabel === 'GST' ? 'SGST' : `S-${selectedBill.taxLabel}`} ({(selectedBill.taxRate / 2).toFixed(1)}%):</span>
                  <span>{formatCurrency(selectedBill.tax / 2)}</span>
                </div>
              </>
            )}
